...
CPCv2 code path verbatim, one resolution up.
    ...
    python scripts/71_v7_coarsen_pack_archive.py \\
...
def create(group, name: str, values=None, **kwargs):
    """Create an array through the one API both zarr generations expose.

    zarr 2 groups expose ``create_dataset`` only; zarr 3 also has a newer
    factory, but its ``create_dataset`` requires an explicit shape.  Passing
    shape and dtype always, through ``create_dataset``, works on both -- and
    matches what ``bdhires.zarr_output`` already does, so this file does not
    introduce a second convention for the same operation.
    """
    if values is not None:
        values = np.ascontiguousarray(values)
        kwargs.setdefault("shape", values.shape)
        kwargs.setdefault("dtype", values.dtype)
        kwargs.setdefault("chunks", values.shape)
...
def main() -> None:
    ...
    create(root, "time", times)
    create(root, "lat", coarse_lat)
    create(root, "lon", coarse_lon)
    create(root, "valid", coarse_valid)
    create(
        root, "static",
        block_reduce(np.asarray(source["static"][:]), area, valid, factor),
    )
    target_array = create(
        root, "target", shape=(ntime, *shape), chunks=(1, *shape), dtype="f4",
    )
    cond_array = create(
        root, "cond", shape=(ntime, len(channels), *shape),
