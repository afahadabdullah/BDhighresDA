...
mkdir -p logs
export V2_BWDB_WINNER_ROOT="${V2_BWDB_WINNER_ROOT:-data/processed/v2_bmd_bwdb_huber3_2021_2024}"
...
prepare_result="$(sbatch --parsable --export=ALL "$@" slurm/v2_bmd_bwdb_huber3_2021_2024_prepare.sbatch)"
prepare_job="${prepare_result%%;*}"
array_result="$(sbatch --parsable --dependency="afterok:${prepare_job}" --export=ALL "$@" slurm/v2_bmd_bwdb_huber3_2021_2024.sbatch)"
array_job="${array_result%%;*}"
summary_result="$(sbatch --parsable --dependency="afterok:${array_job}" --export=ALL "$@" slurm/v2_bmd_bwdb_huber3_2021_2024_summary.sbatch)"
summary_job="${summary_result%%;*}"
echo "submitted preparation: $prepare_job"
echo "submitted GPU array:   $array_job (4 constrained evaluations + 4 all-station analyses)"
echo "submitted summary:     $summary_job"
echo "Zarr:    $V2_BWDB_WINNER_ROOT/gridded/{2021_may_sep,2022_may_sep,2023_may_sep,2024_may_jun}.zarr"
