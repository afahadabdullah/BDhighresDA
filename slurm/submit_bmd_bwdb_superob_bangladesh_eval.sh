#!/usr/bin/env bash
# Submit Bangladesh ADM0 evaluation for BMD+BWDB Superob 2021-2024 data across monthly and multi-month periods.
set -euo pipefail
...
export V2_BWDB_ROOT="${V2_BWDB_ROOT:-data/processed/v2_bmd_bwdb_superob_2021_2024}"
...
METHOD="${V2_BWDB_METHOD:-dense_s6_bwdb_r4}"
PRODUCT_NAME="${BRISHTI_PRODUCT_NAME:-BRISHTI-05 (BMD+BWDB Superob)}"
...
if [[ ! -s "$BOUNDARY" ]]; then
    ...
    if [[ -s "data/processed/v2_bmd_bwdb_huber3_2021_2024/evaluation/brishti05_inputs/geoBoundaries-BGD-ADM0.geojson" ]]; then
        cp "data/processed/v2_bmd_bwdb_huber3_2021_2024/evaluation/brishti05_inputs/geoBoundaries-BGD-ADM0.geojson"* "$(dirname "$BOUNDARY")/"
    elif [[ -s "data/processed/v2_confirmatory_2021_2024/evaluation/brishti05_inputs/geoBoundaries-BGD-ADM0.geojson" ]]; then
...
for spec in "${PERIOD_SPECS[@]}"; do
    ...
    result="$(sbatch --parsable \
        --export="ALL,V2_BWDB_ROOT=$V2_BWDB_ROOT,V2_IMERG_NATIVE_ROOT=$V2_IMERG_NATIVE_ROOT,V2_BWDB_BGD_OUT=$out,V2_BWDB_BGD_BOUNDARY=$BOUNDARY,V2_BWDB_CPC_DIR=$CPC_DIR,V2_BWDB_METHOD=$METHOD,BRISHTI_PRODUCT_NAME=$PRODUCT_NAME,BRISHTI_MONTHS=$months,BRISHTI_PERIOD_TAG=$tag,BRISHTI_YEAR=$TARGET_YEAR" \
        slurm/bmd_bwdb_superob_bangladesh_eval.sbatch)"
    job_id="${result%%;*}"
