...
\paragraph{F6. A confound is present in all three arms.} All three v3
configurations set $\texttt{cond\_dropout}=0.1$, whereas V1 used $0.0$. This was
introduced to enable a later unconditional-prior ablation and is unrelated to
the wet-bias hypothesis. Training with the conditioning dropped on $10\%$ of
batches teaches the model an unconditional branch; the validation samples are
drawn at $w=1$, i.e.\ with no classifier-free guidance applied to recover the
conditional sharpness. The consequence is that a partially unconditional --- and
therefore broader --- predictive distribution is being sampled. This is the only
change common to all three arms and absent from the baseline, and it is
consistent with all three symptoms in F3: growing spread, inflated wet fraction,
and depressed spatial correlation. It is a plausible but \emph{not yet tested}
explanation, and until it is tested the comparison of any v3 arm against V1
confounds the redesign with this change.

\subsection{Limitations}

\begin{enumerate}
  \item \textbf{The confound in F6 is not controlled.} No arm was trained with
    $\texttt{cond\_dropout}=0.0$, so the redesign cannot be cleanly separated
    from that change. This is the principal weakness of the experiment.
...
The ablation supports a specific next configuration rather than a further sweep.
The residual target should be retained (F4), the hurdle head should be kept but
paired with $\texttt{dry\_weight}=2.0$ (F5), and $\texttt{cond\_dropout}$ should
be returned to $0.0$ to remove the confound (F6). Because the wet-frequency
target is achieved transiently and then lost as spread grows (F2, F3), model
selection should additionally track wet-frequency error rather than CRPS alone;
on this evidence CRPS continues to improve while the property of interest
degrades, so a CRPS-selected checkpoint is not the right one.

A cheap test of F6 is available before committing further training: resampling
an existing checkpoint at $w=2$ rather than $w=1$ should sharpen the predictive
distribution if the diffuseness originates in the unconditional branch, and
should leave it unchanged otherwise.
